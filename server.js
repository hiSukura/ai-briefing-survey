...
app.get('/api/results', (req, res) => {
  try {
    ...
    const labels = ['姓名','岗位','客户数量','发简报频率','过去一月做几份','当月几号发','活动周期','全流程耗时','是否有SOP','客户清单','使用场景','各场景客户数','模板类型','前10指标','常用指标','B/C端区分','环比','同比','口径书面定义','获取数据方式','取数等待时间','是否需清洗','取错过数据吗','统一取数接口','使用哪些AI','几个Prompt','AI可用比例','AI编造数据','人工改什么','谁审核','审核关注什么','发给客户方式','客户查看设备','需要PDF吗','客户表达规则','数据缺失处理','客户追问','续费增购案例','客户详细清单','指标口径说明','最难统一部分','期望自动化','样例可提供','优先级3件事','补充说明'];
